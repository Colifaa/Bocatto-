  ...
  return (
    <Chakra.Box >
      ...
      <Chakra.SimpleGrid  bgGradient="linear(to bottom, #FF5733, #F2C94C, #B76840)" templateColumns={{ base: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(3, 1fr)' }} gap={2} justifyContent="center" >
        {cart?.map((producto) => (
          <Chakra.Box key={producto.id} p={4} borderWidth={1} borderRadius="lg" borderColor="red.500" bgColor="#000000" color="#FFFFFF" boxShadow="none" bgImage="Bocatto.png" bgSize="contain" bgPos="center" bgRepeat="no-repeat">
            <Chakra.Text fontSize="xl" color="#B76840">{producto.nombre}</Chakra.Text>
            <Chakra.Text fontSize="xl" color="#FF5733">Precio: ${producto.precio}</Chakra.Text>
            <Chakra.Text fontSize="xl" color="#F2C94C">
              {producto.cantidad > 1 && (
                <span>Cantidad: x{producto.cantidad}</span>
              )}
            </Chakra.Text>
            <Chakra.Text fontSize="xx-large" color="#F2C94C">
